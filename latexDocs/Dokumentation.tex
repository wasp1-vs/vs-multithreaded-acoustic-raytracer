...
\begin{document}
    ...
    \subsection{Umstellung auf 3D: Vec3 und Möller-Trumbore-Algorithmus}

    Im Verlauf des Projekts wurde die ursprüngliche zweidimensionale Physik-Engine auf einen vollständig dreidimensionalen Simulationsraum erweitert. Ziel dieser Umstellung war es, Schallstrahlen nicht mehr ausschließlich in einer Ebene zu simulieren, sondern deren Ausbreitung in realistischen dreidimensionalen Räumen zu ermöglichen.
    Hierfür wurden die Datentypen der zentralen
    Simulationsobjekte (Ray, Mikrofonposition, Lautsprecherposition und Reflexionspunkte) von \texttt{Vec2} auf \texttt{Vec3} migriert. Während ein Punkt zuvor lediglich durch die Koordinaten $(x,y)$ beschrieben wurde, enthält ein Punkt im dreidimensionalen Raum zusätzlich eine Höhenkoordinate $z$:

    \[
    P = (x,y,z)
    \]

    Die Erweiterung auf drei Dimensionen betrifft insbesondere die Positionen von Lautsprechern, Mikrofonen, Reflexionspunkten sowie die Richtungsvektoren der ausgesendeten Schallstrahlen.

    \subsubsection{Ablösung des 2D-Segment-Schnitts}

    Der bisher verwendete Strahl-Segment-Schnitt konnte ausschließlich Kollisionen mit zweidimensionalen Wandsegmenten berechnen. Für eine physikalisch korrekte Simulation in 3D müssen Schallstrahlen jedoch mit dreidimensionalen Flächen interagieren.

    Aus diesem Grund werden Wände nicht mehr als Liniensegmente modelliert, sondern als Dreiecke. Dreiecke stellen die Standardrepräsentation polygonaler Oberflächen in der Computergrafik und Physiksimulation dar und ermöglichen die Darstellung beliebig komplexer Geometrien.

    \newpage
    \subsubsection{Möller-Trumbore-Algorithmus}

    Zur Berechnung von Strahl-Dreieck-Schnittpunkten wurde der Möller-Trumbore-Algorithmus implementiert. Dieser Algorithmus gehört zu den am häufigsten eingesetzten Verfahren im Raytracing, da er Schnittpunkte direkt berechnen kann, ohne zuvor die Ebenengleichung des Dreiecks bestimmen zu müssen.
    Gegeben sind:

    \begin{itemize}
    \item Ursprung des Strahls $O$
    \item Richtungsvektor des Strahls $\vec{D}$
    \item Die drei Eckpunkte des Dreiecks $V_0$, $V_1$ und $V_2$
    \end{itemize}

    Zunächst werden die beiden Kantenvektoren des Dreiecks berechnet:
    ...
    Anschließend wird mithilfe von Kreuz- und Skalarprodukten überprüft, ob der Strahl das Dreieck trifft und ob sich der Schnittpunkt innerhalb der Dreiecksgrenzen befindet.

    Der Algorithmus liefert dabei drei wichtige Parameter:

    \begin{itemize}
    \item $t$ : Entfernung des Schnittpunkts entlang des Strahls
    \item $u$ : baryzentrische Koordinate innerhalb des Dreiecks
    \item $v$ : baryzentrische Koordinate innerhalb des Dreiecks
    \end{itemize}

    Ein gültiger Schnittpunkt liegt nur dann vor, wenn folgende Bedingungen erfüllt sind:
    ...
    Werden diese Bedingungen erfüllt, befindet sich der Schnittpunkt innerhalb des Dreiecks und eine Reflexion kann berechnet werden.

    \vspace{0.3em}
    Der folgende Codeausschnitt zeigt den Kern des Möller-Trumbore-Schnittpunkttests.
    Die vollständige Implementierung befindet sich in
    \texttt{rust\_service/src/geometry.rs} in der Funktion
    \texttt{check\_intersection}.
    ...
    Nach erfolgreicher Kollisionsprüfung wird die Oberflächennormale des Dreiecks bestimmt. Die Oberflächennormale wird zunächst normiert,
    um einen Einheitsnormalenvektor zu erhalten. Diese ergibt sich aus dem Kreuzprodukt der beiden Kantenvektoren:

    \[
    \vec{N} = \vec{E_1} \times \vec{E_2}
    \]

    Die Richtung des reflektierten Strahls wird anschließend durch die allgemeine Reflexionsgleichung berechnet:
    ...
    \vec{D}
    -
    2(\vec{D}\cdot\vec{N})\vec{N}
    \]

    Dadurch entstehen physikalisch plausible Reflexionen an dreidimensionalen Oberflächen.

    \subsubsection{Ergebnis der Umstellung}

    Die erfolgreiche Migration von \texttt{Vec2} auf \texttt{Vec3} bildet die Grundlage für zukünftige Erweiterungen der Physik-Engine. Durch die Implementierung des Möller-Trumbore-Algorithmus können nun beliebige polygonale 3D-Oberflächen verarbeitet werden. Damit wurde die Grundlage geschaffen, realistische dreidimensionale Raumakustik mit beliebig komplexen Polygonnetzen zu simulieren. Die bisherige Beschränkung auf zweidimensionale Wandsegmente wurde vollständig aufgehoben.
...
walls: &Vec<Wall>
    ...
    \subsection{Python-Subprozess Runner und JSON-Ingestion}

    Neben der Rust-Physik-Engine wurde eine Python-Schnittstelle implementiert, um die vorkompilierte Rust-Binary automatisiert aus der Python-Pipeline heraus zu starten. Diese Schnittstelle befindet sich in der Datei \texttt{python\_service/rust\_runner.py} und bildet die Verbindung zwischen der performanten Rust-Simulation und den weiteren Verarbeitungsschritten in Python.

    \vspace{0.3em}
    Die Funktion \texttt{run\_rust\_and\_load\_json} erhält den Pfad zur Rust-Binary sowie den erwarteten Pfad zur Ausgabedatei \texttt{ir\_output.json}. Zunächst werden die übergebenen Pfade geprüft. Existiert die Rust-Binary nicht oder zeigt der Pfad nicht auf eine Datei, wird ein klarer Fehler ausgelöst. Dadurch werden fehlerhafte Konfigurationen früh erkannt.

    \vspace{0.3em}
    Anschließend startet Python die Rust-Binary über das Modul \texttt{subprocess}. Die Rust-Simulation läuft dabei als externer Prozess. Python wartet, bis dieser Prozess vollständig beendet ist, und prüft danach den Rückgabecode. Falls die Rust-Binary mit einem Fehler beendet wird, werden sowohl \texttt{stdout} als auch \texttt{stderr} ausgegeben, um die Fehlersuche zu erleichtern.

    \vspace{0.3em}
    Nach erfolgreicher Ausführung wird geprüft, ob die Datei \texttt{ir\_output.json} tatsächlich erzeugt wurde. Diese Datei enthält die von Rust berechneten Impulsantwort-Daten, insbesondere die Verzögerungszeiten und Druckwerte der empfangenen Schallstrahlen. Die JSON-Datei wird anschließend geöffnet, mit \texttt{json.load} eingelesen und als Python-Dictionary zurückgegeben.


    Der folgende Codeausschnitt zeigt den zentralen Ablauf der Schnittstelle:
    \vspace{0.3em}
    ...
    if result.returncode != 0:
        raise RuntimeError("Rust-Binary ist fehlgeschlagen.")
    ...
    Die Kommunikation zwischen Rust und Python erfolgt über die Datei
    \texttt{ir\_output.json}. Dadurch bleibt die Physik-Simulation in Rust,
    während Python die weitere Verarbeitung übernimmt. Die Lösung ist modular,
    einfach testbar und benötigt keine speziellen Bindings wie \texttt{PyO3}.

    Die Funktion wurde anschließend mit der Testdatei
    \texttt{test\_rust\_runner\_manual.py} überprüft. Dabei wurde bestätigt,
    dass die von Rust erzeugte JSON-Datei erfolgreich geladen wird und die
    Datenstruktur die erwarteten Schlüssel \texttt{metadata} und \texttt{hits}
    enthält.
